...
class ReleaseAdmin(admin.ModelAdmin):
    ...
    def make_featured(self, request, queryset):
        ...
        self.message_user(request, f"{queryset.count()} релизов добавлено в избранное")
    
    @admin.action(description='Убрать из избранного')
    def remove_featured(self, request, queryset):
        queryset.update(is_featured=False)
        self.message_user(request, f"{queryset.count()} релизов убрано из избранного")
...
class TrackAdmin(admin.ModelAdmin):
    list_display = ('track_number', 'title', 'release', 'duration_formatted')
    list_display_links = ('title',)
